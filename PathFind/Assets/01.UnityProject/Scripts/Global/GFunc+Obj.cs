...
{
    //! 특정 오브젝트의 자식 오브젝트를 서치해서 컴포넌트 찾아주는 함수
        ...
        GameObject firstObj, GameObject secondObj) 
    {
        ...
        {
            // x 포지션이 같으면 같은 타일임으로 0을 리턴
            if (fPos.x.IsEquals(sPos.x)) { compareResult = 0; }
            else 
            {
                if (fPos.x.IsEquals(sPos.x)) { compareResult = -1; }
    ...
    #region Object transform controll
